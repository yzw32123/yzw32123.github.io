...
class Base extends Controller
{
	...
	public function wxLogin(){
			...
			$data = Request::instance()->param();
			 if(empty($user)){
				...
				$res = Db::name("driver")->insert([
					...
					'free'=>$data['feiyong'],
					'number'=>$data['number'],
					'chufa_date'=>$data['date'],
					'chufa_time'=>$data['time'],
					'status'=>2,
					'jiesong'=>$data['jiesong'],
					'openid'=>$user['openid'],
					'creat_t'=>time(),
				]);
		...
		//车辆预约
		public function car_yuyue(){
			$user = session("shequ_user");
			$data = Request::instance()->param();
			if(!empty($user)){
				$res = Db::name("car_yuyue")->insert([
